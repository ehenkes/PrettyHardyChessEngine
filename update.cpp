...
int MakeMove(const int start, const int dest)
{
	...
	// If a pawn moves and changes file without making a capture, 
	// then its an en passant capture and the captured pawn is removed.
	if (board[start] == P)
	{
		fifty = 0;
	...
	if (board[dest] < 6) // 0-5 sind Bauern/Figuren, 6 bedeutet leeres Feld
	{
		fifty = 0;
		RemovePiece(xside, board[dest], dest);
	}

	// If a pawn moves to the last rank then its promoted. 
	// The pawn is removed and a queen is added. <-- Wieso immer Dame?
...
int TakeBack()
{
	...
	// Schlagen einer Figur rückgängig
	if (m->capture != EMPTY)
	{
